...
const DetailPage = () => {
  ...
  if (!post) {
    return <Container><p>게시물을 찾을 수 없습니다.</p></Container>;
  }
    ...
    <Container>
      {isEditing ? (
        ...
        <>
          ...
          <Meta>
            <span>by <strong>{post.authorId}</strong></span>
...
const Container = styled.div`
  background: white; padding: 2rem; border-radius: 8px; box-shadow: ${({ theme }) => theme.shadow};
`;
const Title = styled.h1` margin-bottom: 0.5rem; `;
const Meta = styled.div` display: flex; gap: 1rem; color: #6c757d; margin-bottom: 1.5rem; `;
...
const FollowButton = styled.button<{ following: boolean }>`
    ...
    background-color: ${({ following, theme }) => following ? theme.colors.secondary : theme.colors.success};
    color: white;
    margin-bottom: 1rem;
`;
const EditForm = styled.div` display: flex; flex-direction: column; gap: 1rem; `;
const Input = styled.input` padding: 0.8rem; border: 1px solid #dee2e6; border-radius: 4px; font-size: 1.5rem; font-weight: bold; `;
const Textarea = styled.textarea` padding: 0.8rem; border: 1px solid #dee2e6; border-radius: 4px; min-height: 200px; resize: vertical; font-size: 1.1rem; `;
const CommentsSection = styled.div` margin-top: 3rem; border-top: 1px solid #dee2e6; padding-top: 2rem; `;
const CommentForm = styled.form` display: flex; gap: 0.5rem; margin: 1rem 0; `;
const CommentInput = styled.input` flex: 1; padding: 0.8rem; border: 1px solid #dee2e6; border-radius: 4px; `;
const SubmitButton = styled.button` padding: 0.8rem 1rem; background-color: ${({ theme }) => theme.colors.primary}; color: white; border-radius: 4px; `;